...
const getExamData = async () => {
    ...
    try {
        // 获取用户答题记录
        const responseResult = await getResponseDetailsService(props.examId, userInfoStore.info.id)
...
// 提交方法
const submitExam = async (isSaveAction = false) => {
    try {
        // 如果不是保存操作，先验证必答题
        if (!isSaveAction) {
            ...
            <el-skeleton :loading="loading" animated :rows="10">
                <template #default>
                    ...
                    <div class="button-group">
                        <el-button type="primary" @click="submitExam(true)">保存</el-button>
